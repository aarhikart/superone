...
export function getOrCreateCelebrationTransporter() {
  ...
  cachedTransporter = nodemailer.createTransport({
    ...
    secure: true,
    auth: { user, pass },
    maxConnections: 5, // Multiple persistent TLS tunnels for faster parallel sending
    maxMessages: Infinity,
    rateDelta: 1000,
    rateLimit: 10, // Max 10 emails per second across all connections
    tls: {
...
export async function runAutomatedCelebrationCheck({ force = false } = {}) {
  ...
  try {
    ...
    const emailPromises = todayCelebrations.map(async (item) => {
